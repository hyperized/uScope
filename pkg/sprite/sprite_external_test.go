...
// TestArrowShape checks the arrow's fixed points directly, independent of
// Draw: the tip, the centre, the head's widest row, a clear corner, and the
// mirror symmetry the point-up art has about its vertical axis.
//
//nolint:varnamelen // x, y is the pixel-addressing idiom used throughout uScope.
func TestArrowShape(t *testing.T) {
	t.Parallel()
	...
	if got := bmp.Side(); got != wantSide {
		t.Fatalf("Arrow().Side() = %d, want %d", got, wantSide)
	}

	const centre = 4

	if !bmp.At(centre, 0) {
		t.Error("At(4, 0) (tip) = false, want true")
	}

	if !bmp.At(centre, centre) {
		t.Error("At(4, 4) (centre) = false, want true")
	}
	...
	for x := headLeft; x <= headRight; x++ {
		if !bmp.At(x, headRow) {
			t.Errorf("At(%d, %d) (head's widest row) = false, want true", x, headRow)
		}
	}

	if bmp.At(0, 0) {
		t.Error("At(0, 0) (corner) = true, want false")
	}

	for y := range wantSide {
		for x := range wantSide {
			mirrorX := wantSide - 1 - x
			if bmp.At(x, y) != bmp.At(mirrorX, y) {
				t.Errorf("At(%d, %d) = %v, At(%d, %d) = %v, want equal (vertical symmetry)",
					x, y, bmp.At(x, y), mirrorX, y, bmp.At(mirrorX, y))
			}
		}
	}
}

// TestArrowDrawRotation is the check that matters to a caller: draw the
// arrow at a known centre and confirm the tip lands where compass heading
// says it should, at each axis-aligned heading.
//
// The pixel diametrically opposite the tip is not a useful "should be
// clear" check here: the shaft runs to the bottom row, so at side 9 its far
// end sits at distance 4 from the centre too, the same distance as the tip.
// Each case checks the right-hand end of the head's widest row instead,
// which lands at a different offset for every heading and so does tell the
// four cases apart.
func TestArrowDrawRotation(t *testing.T) {
	t.Parallel()
	...
	for _, testCase := range []struct {
		...
		{
			name: "0 degrees", heading: heading0,
			tipDX: 0, tipDY: -tipOffset,
			wideDX: headHalfWidth, wideDY: -headRowOffset,
		},
		{
			name: "90 degrees", heading: heading90,
			tipDX: tipOffset, tipDY: 0,
			wideDX: headRowOffset, wideDY: headHalfWidth,
		},
		{
			name: "180 degrees", heading: heading180,
			tipDX: 0, tipDY: tipOffset,
			wideDX: -headHalfWidth, wideDY: headRowOffset,
		},
		{
			name: "270 degrees", heading: heading270,
			tipDX: -tipOffset, tipDY: 0,
			wideDX: -headRowOffset, wideDY: -headHalfWidth,
		},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			...
			sprite.Arrow().Draw(canv, centre, centre, testCase.heading, canvas.White)

			tipX, tipY := centre+testCase.tipDX, centre+testCase.tipDY
			if got := canv.Image().RGBAAt(tipX, tipY); got != canvas.White {
				t.Errorf("tip (%d, %d) = %v, want painted", tipX, tipY, got)
			}

			wideX, wideY := centre+testCase.wideDX, centre+testCase.wideDY
			if got := canv.Image().RGBAAt(wideX, wideY); got != canvas.White {
				t.Errorf("head widest-row point (%d, %d) = %v, want painted", wideX, wideY, got)
			}
		})
	}
}

// TestArrowIsNotTheAirplane guards against a caller getting one built-in
// sprite when it asked for the other: the two accessors must not collapse
// onto the same bitmap or report the same size.
func TestArrowIsNotTheAirplane(t *testing.T) {
	t.Parallel()

	if sprite.Arrow() == sprite.Airplane() {
		t.Fatal("Arrow() and Airplane() returned the same bitmap")
	}

	arrowSide, airplaneSide := sprite.Arrow().Side(), sprite.Airplane().Side()
	if arrowSide == airplaneSide {
		t.Errorf("Arrow().Side() = %d, Airplane().Side() = %d, want different", arrowSide, airplaneSide)
	}
}
